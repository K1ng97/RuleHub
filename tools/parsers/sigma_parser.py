...
class SigmaParserConfig:
    """Sigma解析器配置类"""
    def __init__(self):
        self.project_root = Path(__file__).parent.parent.parent
        self.rules_dir = self.project_root / Path('rules/sigma')
        self.output_path = self.project_root / Path('rules/sigma/sigma_metadata.json')

class SigmaParser:
    """Sigma规则解析器类"""
    def __init__(self, config: SigmaParserConfig):
        self.config = config
        logging.basicConfig(
            level=logging.INFO,
    ...
    def parse(self, file_path: Path) -> Optional[Dict]:
        ...
        try:
            ...
            self.logger.error(f"解析Sigma规则文件 {file_path} 失败: {e}")
            return None
...
if __name__ == '__main__':
    ...
    logger.info(f"共解析 {len(metadata_list)} 条Sigma规则元数据")
